...
# Install uv
subprocess.run([sys.executable, "-m", "pip", "install", "uv"], check=True, shell=True)

# Create virtual environment
subprocess.run(["uv", "venv"], check=True, shell=True)

# Install packages in the virtual environment
subprocess.run(["uv", "pip", "install", "."], check=True, shell=True)

command_args = [r".venv\Scripts\python", "-m", "robot_framework"] + sys.argv[1:]

subprocess.run(command_args, check=True, shell=True)
